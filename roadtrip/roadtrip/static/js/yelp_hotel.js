// yelp api to extract hotel information
...
$.ajax({
  ...
  'success': function(data, text, XMLHttpRequest) {
	...
	for (var j = 0; j < data.businesses.length; ++j){
		...
		console.log('hotel_name',current_business_name);
		
		$('.hotel-item:last').clone().appendTo('.hotel-list').find(".hotel-name-en").html(current_business_name);
		$('.hotel-item:first').hide();
		}
	}
});
